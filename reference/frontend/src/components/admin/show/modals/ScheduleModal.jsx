...
import { 
  ...
  Trash2, ChevronLeft, ChevronRight, CalendarX, Edit2, AlertTriangle 
} from 'lucide-react';
    ...
    <ModalPortal>
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-6xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 border" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
                            ...
                            {schedules.filter(s => s.day === d).map(slot => { 
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {/* DELETE (Opens Popup) */}
                                  <button 
                                    onClick={(e) => { e.stopPropagation(); handleDeleteClick(slot._id); }} 
          ...
          {deleteId && (
            <div className="absolute inset-0 z-[250] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
              <div 
                className="w-full max-w-sm p-6 rounded-2xl shadow-2xl border text-center transform scale-100 animate-in zoom-in-95" 
              ...
              >
                ...
                <h3 className="text-xl font-bold mb-2" style={{ color: 'var(--text-primary)' }}>Delete Schedule?</h3>
                <p className="text-sm mb-6 opacity-70" style={{ color: 'var(--text-secondary)' }}>
                  Are you sure you want to permanently remove this time slot? This action cannot be undone.
                </p>
                <div className="flex gap-3">
                  <button 
                    onClick={() => setDeleteId(null)} 
                    className="flex-1 py-3 font-bold rounded-xl border hover:bg-[var(--bg-secondary)] transition-colors" 
                  ...
                  <button 
                    ...
                    className="flex-1 py-3 font-bold rounded-xl bg-red-500 text-white shadow-lg hover:bg-red-600 transition-colors"
                  >
                    Yes, Delete
                  </button>
                </div>
