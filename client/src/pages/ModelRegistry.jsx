...
export default function ModelRegistry() {
    ...
    const handleExport = async (versionId, modelName, versionNum) => {
        try {
            toast.info("Preparing Secure Export", "Building CycloneDX ML-BOM...");
            await exportModelVersion(versionId, modelName, versionNum);
            toast.success("Export Complete", "Signed ZIP downloaded.");
        } catch (err) {
            toast.error("Export Failed", "Could not generate ML-BOM.");
        }
    };
                                ...
                                <div key={ver.id} className="flex items-center justify-between p-3 bg-surface rounded-xl border border-border group hover:border-primary/30 transition-colors">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            {/* ✨ INTEGRATED: Zero-Trust Resilience Score Badge */}
                                            <div className="flex items-center space-x-2 mt-1.5">
                                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-green-900/30 text-green-400 border border-green-500/50 shadow-[0_0_10px_rgba(34,197,94,0.2)]">
                                                    <svg className="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                        <path fillRule="evenodd" d="M2.166 4.999A11.954 11.954 0 0010 1.944 11.954 11.954 0 0017.834 5c.11.65.166 1.32.166 2.001 0 5.225-3.34 9.67-8 11.317C5.34 16.67 2 12.225 2 7c0-.682.057-1.35.166-2.001zm11.541 3.708a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                                                    </svg>
                                                    ZT_res: 1.00 (Verified)
                                                </span>
                                                <span className="text-[10px] text-gray-500 italic">Air-Gapped &amp; AST Secured</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {!isTrash && (
                                        <div className="flex gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            {/* ✨ INTEGRATED: Export CycloneDX ML-BOM Button */}
                                            <button
                                                onClick={(e) => { e.stopPropagation(); onExport(ver.id, model.name, ver.version); }}
